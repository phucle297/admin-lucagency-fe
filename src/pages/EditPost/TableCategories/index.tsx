...
const TableCategories: FC<ITabelCategoriesProps> = ({
  ...
  useEffect(() => {
    ...
    const listSelected = defaultCategories.map((name: string) => {
      const row = categories?.find((item: ICategory) => item?.name === name);
      return {
        ...row,
      };
    });
    setSelectedCategories(listSelected as ICategory[]);
    setSelectedRowKeys(listSelected.map((item) => item._id as string));
